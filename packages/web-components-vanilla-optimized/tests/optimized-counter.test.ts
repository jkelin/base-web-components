...
function mount(inner: string): HTMLElement {
  // This insertion connects the parent before child upgrades, matching parser order.
...
describe("optimized vanilla counter", () => {
  afterEach(() => {
  ...
  it("renders light-DOM children and counts uncontrolled", () => {
    ...
    expect((minus as HTMLElement).style.cursor).toBe("pointer");
