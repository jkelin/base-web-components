...
export function h(tag: string, props: Record<string, unknown>, ...children: Child[]): HTMLElement {
  ...
  for (const [name, value] of Object.entries(props ?? {})) {
    ...
    else if (name in element) (element as unknown as Record<string, unknown>)[name] = value;
    else element.setAttribute(name, value === true ? "" : String(value));
  }
  const append = (child: Child): void => {
    if (Array.isArray(child)) child.forEach(append);
    else if (child != null) element.append(child);
  };
  children.forEach(append);
  return element;
}
export function render(node: Node, host: HTMLElement): void {
  host.replaceChildren(node);
}
export abstract class LightLitElement extends LitElement {
...
export abstract class ObservedRoot extends LightLitElement {
...
export class TextButton extends LightLitElement {
  static override observedAttributes = ["class", "disabled"];
...
export abstract class OpenRoot extends ObservedRoot {
  ...
  #controlled = false;
  #initialized = false;
  static override observedAttributes = ["open", "disabled"];
  get open() {
    return this.#open;
  }
