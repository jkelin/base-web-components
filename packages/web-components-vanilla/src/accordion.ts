...
export const VANILLA_ACCORDION_TAG = "vanilla-accordion";
export const VANILLA_ACCORDION_ITEM_TAG = "vanilla-accordion-item";
export const VANILLA_ACCORDION_TRIGGER_TAG = "vanilla-accordion-trigger";
export const VANILLA_ACCORDION_PANEL_TAG = "vanilla-accordion-panel";

export class VanillaAccordionItemElement extends HTMLElement {
  get value(): string {
...
export class VanillaAccordionTriggerElement extends ControlLeaf {
  static override marker = "accordion-trigger";
}
export class VanillaAccordionPanelElement extends HTMLElement {}

export class VanillaAccordionElement extends DeferredElement {
  ...
  #initialized = false;
  #onClick = (event: Event) => {
    const button = (event.target as Element).closest(`${VANILLA_ACCORDION_TRIGGER_TAG} > button`);
    if (!button || !this.contains(button) || this.disabled) return;
    const item = button.parentElement?.parentElement;
    if (!(item instanceof VanillaAccordionItemElement) || item.hasAttribute("disabled")) return;
  ...
  protected sync(): void {
    const items = [
      ...this.querySelectorAll<VanillaAccordionItemElement>(VANILLA_ACCORDION_ITEM_TAG),
    ];
    ...
    for (const item of items) {
      const trigger = item.querySelector<VanillaAccordionTriggerElement>(
        `:scope > ${VANILLA_ACCORDION_TRIGGER_TAG}`,
      );
      const panel = item.querySelector<VanillaAccordionPanelElement>(
        `:scope > ${VANILLA_ACCORDION_PANEL_TAG}`,
      );
      const button = trigger?.querySelector("button");
      if (!trigger || !panel || !button) continue;
      const open = this.#value.includes(item.value);
      button.id ||= nextId(`${VANILLA_ACCORDION_TRIGGER_TAG}-button`);
      panel.id ||= nextId(VANILLA_ACCORDION_PANEL_TAG);
...
const definitions: Array<[string, CustomElementConstructor]> = [
  [VANILLA_ACCORDION_TAG, VanillaAccordionElement],
  [VANILLA_ACCORDION_ITEM_TAG, VanillaAccordionItemElement],
  [VANILLA_ACCORDION_TRIGGER_TAG, VanillaAccordionTriggerElement],
  [VANILLA_ACCORDION_PANEL_TAG, VanillaAccordionPanelElement],
];
