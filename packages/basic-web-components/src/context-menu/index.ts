...
if (typeof document !== "undefined" && !document.getElementById("bwc-context-menu-style")) {
  const style = document.createElement("style");
  style.id = "bwc-context-menu-style";
...
const anchorModes = ["trigger", "pointer"] as const;
type AnchorMode = (typeof anchorModes)[number];
...
export type BwcContextMenuElement = HTMLElement & {
  open: boolean;
  defaultOpen: boolean;
  disabled: boolean;
  ...
  anchor: AnchorOption;
  /** Anchor at the trigger element or at the pointer position. Default pointer. */
...
export const BwcContextMenuElement = defineComponent<BwcContextMenuElement>(
  "bwc-context-menu",
  () => {
    const host = useHost<BwcContextMenuElement>();
    ...
    const anchorMode = useProp<AnchorMode>(
      "anchorMode",
      enumProp("anchor-mode", anchorModes, "pointer"),
    );
    ...
    // Null means the current open request has no pointer anchor; imperative,
    // hover, and default opens then fall back to the trigger.
    ...
    // `show()`/`close()` rather than an `open()` method). Imperative opens
    // have no pointer event, so pointer mode sensibly falls back to trigger.
    host.show = () => {
      if (disabled()) return;
      pointerPosition = null;
      requestOpen(true, false, true);
    };
    ...
    onMount(() => {
      ...
      const contextmenu = (event: MouseEvent) => {
        ...
        if (target instanceof Element && currentParts.trigger.contains(target)) {
          event.preventDefault();
          pointerPosition = { x: event.clientX, y: event.clientY };
          if (!openState()) {
            requestOpen(true);
            queueMicrotask(() => highlightFirstOrLast(true));
          }
        }
      };
      const click = (event: MouseEvent) => {
        ...
        if (currentParts.trigger.contains(target)) return;
      ...
      const mouseover = (event: MouseEvent) => {
        ...
        if (item && currentParts.popup.contains(item) && belongsToHost(item, host)) {
          if (!isMenuItemDisabled(item))
            highlight(menuItems(host, currentParts.popup).indexOf(item));
        }
      };
      const keydown = (event: KeyboardEvent) => {
        ...
        if (currentParts.trigger.contains(target) && !currentParts.popup.contains(target)) {
          if (event.key === "ContextMenu" || (event.shiftKey && event.key === "F10")) {
            event.preventDefault();
            const rect = currentParts.trigger.getBoundingClientRect();
            pointerPosition = { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
            if (!openState()) {
              requestOpen(true);
              queueMicrotask(() => highlightFirstOrLast(true));
            }
      ...
      try {
        ...
        stopTopology = effect(() => {
          ...
          // The region trigger accepts any element (unlike menu's button).
          const candidates = [...host.children].filter(
            (element) => element.getAttribute("slot") === "trigger",
          );
          if (candidates.length !== 1 || !(candidates[0] instanceof HTMLElement)) {
            throw new TypeError("bwc-context-menu requires exactly one element in slot trigger");
          }
          const trigger = candidates[0] as HTMLElement;
          popup.id ||= nextId("bwc-context-menu-popup");
          popup.dataset.testid ||= "bwc-context-menu-popup";
          ...
          trigger.id ||= nextId("bwc-context-menu-trigger");
        ...
        stopClasses = effect(() => {
          ...
          const currentParts = parts();
          if (!currentParts) return;
          // The region trigger may be any element, so skip button-only
          // decoration and only ensure identity selectors.
          currentParts.trigger.id ||= nextId("bwc-context-menu-trigger");
          currentParts.trigger.dataset.testid ||= "bwc-context-menu-trigger";
          applyPartClass(currentParts.trigger, "context-menu-trigger", triggerClass());
          applyPartClass(currentParts.popup, "context-menu-popup", popupClass());
        });
        stopState = effect(() => {
          const currentParts = parts();
          if (!currentParts) return;
          ...
          if (trigger instanceof HTMLButtonElement && trigger.disabled !== isDisabled) {
          ...
          for (const group of popup.querySelectorAll<HTMLElement>("[data-group]")) {
            ...
            if (label && belongsToHost(label, host)) {
              label.id ||= nextId("bwc-context-menu-group-label");
        ...
        stopBehavior = effect(() => {
          ...
          const currentAnchorMode = anchorMode();
          ...
          if (!isOpen) return;
          const { popup, trigger } = currentParts;
          // Pointer mode uses the current contextmenu/keyboard point. Opens
          // without such an event fall back to the trigger; an explicit
          // `anchor` always wins.
          const point = pointerPosition;
          const pointerAnchor = point
            ? ({
            ...
            : null;
          const anchorEl =
            currentAnchorMode === "pointer" && !currentAnchor && pointerAnchor
              ? pointerAnchor
              : (resolveAnchor(host, currentAnchor, trigger) ?? trigger);
          const update = () => {
            const arrow = popup.querySelector<HTMLElement>("[data-arrow]");
          ...
          const stopTracking = trackAnchor(trigger, popup, update);
